...
def connect_to_server(server_name):
    ...
    # Obtener la dirección IP del servidor llamado server_name
    ip_address = socket.gethostbyname(server_name)
    sys.stderr.write("Contactando al servidor en %s...\n" % ip_address)

    # Se crea el socket
    connection = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # Se conecta al servidor utilizando la ip_address y el puerto correcto
    connection.connect((ip_address, HTTP_PORT))

    # Retorna el socket que acaba de conectarse al servidor
    return connection
