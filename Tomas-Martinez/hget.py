...
def connect_to_server(server_name):
    ...
    ip_address = socket.gethostbyname(server_name) #  gethostbyname() obtiene la dirrecion ip asociada con ese nombre de host
    ...
    # Crear socket
    # COMPLETAR ABAJO DE ESTA LINEA
    our_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #creamos un socket con socket.socket donde le pasamos socket.AF_INET que es una familia de directorios y socket.SOCK_STREAM el tipo de socket que se creara en este caso para TCP
    # Aqui deben conectarse al puerto correcto del servidor
    our_socket.connect((ip_address, HTTP_PORT)) ## aca puedo o bien pasar 80 o HTTP_PORT pues es lo mismo, pero lo que hace es justamento conectar al puerto correcto HTTP
    return our_socket
